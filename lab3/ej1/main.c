...
int main(int argc, char *argv[]) {
    ...
    /*show the minimium historic temperature */
    int hist_min = 0;
    hist_min = min_temp_min(array);
    printf("\nThe minimium historic temperature was: %d\n", hist_min);

    /* show the maximum temperature each year */
    int max_temp[YEARS];
    max_temp_max(array, max_temp);
    for (unsigned int i = 0; i < YEARS; i++){
	    printf("The maximum temperature of %d was: %d\n", (FST_YEAR + i), max_temp[i]);
    }

    /* show the maximum rainfall month for each year */
    month_t array_of_months[YEARS];
    max_rainfall_month(array, array_of_months);
    for (unsigned int i = 0u; i < YEARS; i++){
	    printf("The maximun rainfall month of %d was: %d\n", (FST_YEAR + i), (array_of_months[i] + 1));
    }
